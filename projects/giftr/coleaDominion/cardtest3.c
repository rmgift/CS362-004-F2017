...
int main() 
{
	...
	returnVal = cardEffect(village, 0, 0, 0, &copyOfG, 0, &bonus);
	if (returnVal != 0)
	...
	printf("    Current Played: %d\t\tExpected Played: %d\n", copyOfG.playedCards[copyOfG.playedCardCount - 1], k[7]);
	if (copyOfG.playedCards[copyOfG.playedCardCount - 1] != k[7])
	{
		array[index] = 1;
		count++;
	}
	...
	printf("    Current Actions: %d\t\tExpected Actions: %d\n", copyOfG.numActions, (currentG.numActions + 2 - 1));
	if (copyOfG.numActions != (currentG.numActions + 2 - 1))
	{
		array[index] = 1;
		count++;
	}
